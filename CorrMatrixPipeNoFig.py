...
#        print mic[i][j]
        sys.stdout.write(".")
        g=open(output_dir+'/'+'CorrMatrixNMic_'+str(Mdim)+'_'+filename+'.txt','a')
        if j==Mdim:
            g.write(str(mic[i][j]))
            g.write('\n')
        else:
...
#                                                                                                                                
# =============================================================================
f=np.loadtxt(output_dir+'/'+'CorrMatrixNMic_'+str(Mdim)+'_'+filename+'.txt').reshape([Mdim,Mdim,])
...
for i in range(0,len(f)/attrib):
    for j in range(0,len(f)/attrib):
        ...
        k=open(output_dir+'/'+'MaxRedMatrixNMIC'+filename+'.txt','a')
        MaxMIC=np.max(F)
        if j == len(f)/attrib-1:
            k.write(str(MaxMIC))
            k.write('\n')
...
if thv == 0.0 :
    ...
    M=np.loadtxt(output_dir+'/'+'MaxRedMatrixNMIC'+filename+'.txt')
    f=open(output_dir+'/'+'MaxRedMatrixNMIC_'+filename+'_th'+str(thv)+'.txt','a')
        ...
        for j in range(len(M)):
            if j==len(M)-1:
                ...
                if M[i][j]>thv:
                    ...
                    f.write(' ')
    f.close()
    g=open(output_dir+'/'+'GetIndexThrshd_'+filename+'_th'+str(thv)+'.txt','a')
    for i in range(len(M)):
        for j in range(len(M)):
            if i==j:
                pass
...
#fig.savefig(output_dir+'/'+'CMapNMICG.png',dpi=1024)
#plt.show()                            
