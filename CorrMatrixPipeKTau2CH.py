...
parser=OptionParser(usage="Computing Kentalltau and generating correlation matrix with a given data", version="1.000")
...
sys.stdout = Log(sys.stdout, output_dir+'/'+'Ktau_Report_'+str(GPS)+'_'+nfilename+'.log')
...
#
# =============================================================================
print 'Computing Kendalltau and generating correlation matrix...'
for i in range(Mdim):
#    sys.stdout.write(".")
    for j in range(Mdim):
        ktau[i][j] = sp.stats.kendalltau(f1.T[i],f2.T[j])[0]
        sys.stdout.write(".")
        g=open(output_dir+'/'+'CorrMatrixKtau_'+str(Mdim)+'_'+str(GPS)+'_'+nfilename+'.txt','a')
        if j==Mdim-1:
            g.write(str(ktau[i][j]))
            g.write('\n')
        else:
            g.write(str(ktau[i][j]))
            g.write(' ')
...
#                                                                                                                                
# =============================================================================
f=np.loadtxt(output_dir+'/'+'CorrMatrixKtau_'+str(Mdim)+'_'+str(GPS)+'_'+nfilename+'.txt').reshape([Mdim,Mdim,])
...
for i in range(0,len(f)/attrib):
    for j in range(0,len(f)/attrib):
        ...
        k=open(output_dir+'/'+'MaxRedMatrixKtau_'+str(GPS)+'_'+nfilename+'.txt','a')
        Maxktau=np.max(F)
        if j == len(f)/attrib-1:
            k.write(str(Maxktau))
            k.write('\n')
...
if thv == 0.0 :
    ...
    M=np.loadtxt(output_dir+'/'+'MaxRedMatrixKtau_'+str(GPS)+'_'+nfilename+'.txt')
    f=open(output_dir+'/'+'MaxRedMatrixKtau_'+str(GPS)+'_'+nfilename+'_th'+str(thv)+'.txt','a')
...
if thv == 0.0:
    dat = np.loadtxt(output_dir+'/'+'MaxRedMatrixKtau_'+str(GPS)+'_'+nfilename+'.txt')
else:
    dat = np.loadtxt(output_dir+'/'+'MaxRedMatrixKtau_'+str(GPS)+'_'+nfilename+'_th'+str(thv)+'.txt')
...
fig.suptitle('Correlation Matrix via Kentalltau Correlation Coefficient between SEI and GW channels of CLIO', fontsize=25, fontweight='bold')
ax.invert_yaxis()
ax.xaxis.tick_top()
...
fig.savefig(output_dir+'/'+'CMatrixKtau_SEI_GW_CLIO_'+str(GPS)+'_'+nfilename+'.png',dpi=256)
#plt.show()                            
