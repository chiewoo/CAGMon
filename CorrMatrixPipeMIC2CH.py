...
#    sys.stdout.write(".")
    for j in range(Mdim):
        mic[i][j] = normalized_mutual_info_score(f1.T[i],f2.T[j])
#        print mic[i][j]
        sys.stdout.write(".")
        g=open(output_dir+'/'+'CorrMatrixNMic_'+str(Mdim)+'_'+str(GPS)+'.txt','a')
        if j==Mdim:
            g.write(str(mic[i][j]))
            g.write('\n')
        else:
...
#                                                                                                                                
# =============================================================================
f=np.loadtxt(output_dir+'/'+'CorrMatrixNMic_'+str(Mdim)+'_'+str(GPS)+'.txt').reshape([Mdim,Mdim,])
...
for i in range(0,len(f)/attrib):
    for j in range(0,len(f)/attrib):
        ...
        k=open(output_dir+'/'+'MaxRedMatrixNMIC_'+str(GPS)+'.txt','a')
        MaxMIC=np.max(F)
        if j == len(f)/attrib-1:
            k.write(str(MaxMIC))
            k.write('\n')
...
if thv == 0.0 :
    ...
    M=np.loadtxt(output_dir+'/'+'MaxRedMatrixNMIC_'+str(GPS)+'.txt')
    f=open(output_dir+'/'+'MaxRedMatrixNMIC_'+str(GPS)+'_th'+str(thv)+'.txt','a')
...
if thv == 0.0:
    dat = np.loadtxt(output_dir+'/'+'MaxRedMatrixNMIC_'+str(GPS)+'.txt')
else:
    dat = np.loadtxt(output_dir+'/'+'MaxRedMatrixNMIC_'+str(GPS)+'_th'+str(thv)+'.txt')
...
fig.savefig(output_dir+'/'+'CMatrixNMICG_SEI_GW_CLIO_FDA_'+str(GPS)+'.png',dpi=256)
#plt.show()                            
